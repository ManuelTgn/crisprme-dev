...
def _add_common_search_arguments(
    required_group: _ArgumentGroup, optional_group: _ArgumentGroup
) -> None:
    """Add the search arguments shared by every complete-search variant.

    Only the input-source arguments differ between variants (``--genome`` /
    ``--vcf`` for the reference search, the four assembly/chain folders for the
    assembly search); everything else lives here so the two parsers stay in sync.
    Groups are passed in already created so each parser controls where its own
    input-source arguments appear (they precede these).
    """
    # --- required side (paste verbatim, in this order) ---
...
def create_search_parser(subparser: _SubParsersAction) -> Crisprme2ArgumentParser:
    ...
    """
    parser_search: Crisprme2ArgumentParser = subparser.add_parser(
        COMMANDS[0],
        usage="CRISPRme2 search {version}\n\nUsage:\n"
        "\ncrisprme2 search --genome <genome-dir> --vcf <vcf-dir> "
    ...
    general_group: _ArgumentGroup = parser_search.add_argument_group("General options")
    general_group.add_argument(
        "-h", "--help", action="help", help="show this help message and exit"
    )
    required_group: _ArgumentGroup = parser_search.add_argument_group("Options")
    required_group.add_argument(
        "--genome",
        type=str,
        metavar="GENOME-DIR",
...
def create_assembly_search_parser(
    subparser: _SubParsersAction,
) -> Crisprme2ArgumentParser:
    parser_asm: Crisprme2ArgumentParser = subparser.add_parser(
        COMMANDS[1],
        usage="CRISPRme2 search-assemblies {version}\n\nUsage:\n"
        "\ncrisprme2 search-assemblies --assemblies <assemblies-dir> "
        "--chains <chains-dir> --guide <guide> --pam <pam> "
        "--outdir <output-dir>\n\n",
        description="End-to-end off-targets search over one or more genome "
        "assemblies of any ploidy (monoploid, diploid, or polyploid), with "
        "per-haplotype liftover to a reference coordinate system, cross-sample "
        "merging, scoring, and annotation of results",
        help="perform an off-targets search across one or more assemblies of any "
        "ploidy (e.g. HPRC diploid samples or polyploid plant assemblies), "
        "reporting hits in reference (e.g. GRCh38) coordinates with the "
        "contributing samples, scores, and annotations",
        add_help=False,
    )
    general_group: _ArgumentGroup = parser_asm.add_argument_group("General options")
    general_group.add_argument(
        "-h", "--help", action="help", help="show this help message and exit"
    )
    required_group: _ArgumentGroup = parser_asm.add_argument_group("Options")
    required_group.add_argument(
        "--assemblies",
        type=str,
        metavar="ASSEMBLIES-DIR",
        required=True,
        dest="assemblies",
        help="folder of haplotype assembly FASTA files (plain or bgzipped), one "
        "file per haplotype, for any number of samples and any ploidy "
        "(monoploid, diploid, polyploid). Sequence headers must follow PanSN "
        "naming 'sample#haplotype#contig' (e.g. HG06807#2#CM101429.1); each "
        "file's sample and haplotype identity is read from its headers",
    )
    required_group.add_argument(
        "--chains",
        type=str,
        ...
        required=True,
        dest="chains",
        help="folder of chain files lifting each haplotype assembly to the "
        "reference (e.g. GRCh38), one per haplotype. Files are matched to their "
        "haplotype by filename prefix '<sample-id>_hap<hap-id>_' (e.g. "
        "HG06807_hap2_to_grch38.chain); used for liftover and reference annotation",
    )
...
def create_parser_crisprme2() -> Crisprme2ArgumentParser:
    ...
    create_assembly_search_parser(subparsers)
    return parser
