...
class Target:
    ...
    def __init__(self, loggers: CrisprmeLoggers) -> None:
        ...
        self._alignments : List[bytes] = []  # target alignments stored as raw CIGAR ASCII

    def __repr__(self) -> str:
        return (
            f"<{self.__class__.__name__} object; coordinate={len(self.coordinates)}>"
        )
    ...
    def add_target(self, contig: str, position: int, strand: bool) -> None:
        ...
        self._coordinates.add(Coordinate(contig, position + 1, int(not strand), self._loggers))
