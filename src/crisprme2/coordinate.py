...
class Coordinate:
    def __init__(self, contig: str, position: int, strand: int, loggers: CrisprmeLoggers) -> None:
        self._loggers = loggers  # store loggers
        if not isinstance(contig, str) or not contig:
            self._loggers.errorlog.log_raise_exception("Coordinate contig must be a non-empty string", os.EX_DATAERR, TypeError)
            raise ValueError("Contig must be a non-empty string.")
        self._contig: str = contig  # contig name
        if not isinstance(position, int):
            self._loggers.errorlog.log_raise_exception(f"Coordinate position must be an integer, got {type(position).__name__}", os.EX_DATAERR, TypeError)
            raise TypeError("Position must be an integer.")
        if position < 0:
            self._loggers.errorlog.log_raise_exception(f"Coordinate position must be non-negative, got {position}", os.EX_DATAERR, TypeError)
            raise ValueError("Position must be non-negative.")
        self._position: int = position
        if not isinstance(strand, int):
            self._loggers.errorlog.log_raise_exception(f"Coordinate strand must be an integer, got {type(strand).__name__}", os.EX_DATAERR, TypeError)
            raise TypeError("Strand must be an integer.")
        # standard convention: 0 for FWD (+), 1 for REV (-)
        if strand not in {0, 1}:
            self._loggers.errorlog.log_raise_exception(f"Coordinate strand must be 0 (forward) or 1 (reverse), got {strand}", os.EX_DATAERR, TypeError)
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            self._contig == other._contig and
