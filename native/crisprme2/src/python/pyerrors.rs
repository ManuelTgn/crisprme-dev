...
impl From<AnnotationError> for PyErr {
    fn from(err: AnnotationError) -> PyErr {
        match err {
            AnnotationError::IoError(msg) => PyIOError::new_err(msg),

            AnnotationError::MalformedBed { line } => {
                PyValueError::new_err(format!("Malformed BED file at line {}", line))
            }

            AnnotationError::InvalidFeatureId(id) => {
                PyValueError::new_err(format!("Invalid feature ID {}", id))
            }
...
impl From<PamError> for PyErr {
    fn from(err: PamError) -> PyErr {
        match err {
            ...
            PamError::IndexOutOfRange { .. } => PyIndexError::new_err(err.to_string()),
        }
    }
}
