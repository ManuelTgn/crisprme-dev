...
// Follows same implementation of rust `Cigarx128`
struct Cigarx128
{
    // Number of bits for the type
    static constexpr u32 BITS = sizeof(u64) * 8 * 2;
    // Number of operations upported
    static constexpr u32 CAPACITY = (BITS - 1) / 2;
    ...
    __device__ __forceinline__ void push(Oper value)
    {
        ...
        assert(len() < CAPACITY);
#endif
        m_hi = (m_hi << 2) | (m_lo >> (64 - 2));
        m_lo = (m_lo << 2) | static_cast<u64>(value);
    }
    ...
    __device__ __forceinline__ u32 len()
    {
        u32 aclz = m_hi != 0 ? __clzll(m_hi) : 64 + __clzll(m_lo);
        u32 bits = BITS - aclz - 1;
        return bits / 2;
    }
};

__device__ const Cigarx128::Oper CIGARX_ENCODE_128[8] = {
    Cigarx128::Oper::X, // 0b0'00 invalid!
    Cigarx128::Oper::D, // 0b0'01 OK
    Cigarx128::Oper::I, // 0b0'10 OK
    Cigarx128::Oper::M, // 0b0'11 OK
    Cigarx128::Oper::X, // 0b1'00 invalid!
    Cigarx128::Oper::D, // 0b1'01 OK
    Cigarx128::Oper::I, // 0b1'10 OK
    Cigarx128::Oper::X, // 0b1'11 OK
};
...
struct Step
{
    ...
    /// Convert the step to a CIGARX value
    __device__ __forceinline__ Cigarx128::Oper to_cigarx_op_u128(bool mismatch)
    {
        u32 index = (mismatch << 2) | value;
        return CIGARX_ENCODE_128[index];
    }
...
struct ThreadMiner128
{
    ...
    __device__ __forceinline__ ThreadMiner128()
    {
        mem.stack_lo = 0;
        mem.stack_hi = 0;
        mem.mismatches = 0;
    ...
    __device__ __forceinline__ void replace(Step s)
    {
        mem.stack_lo = (mem.stack_lo & ~0b11) | (s.value & 0b11);
    }

    /// Push a new step on top of the stack
    __device__ __forceinline__ void push(Step s, bool set_mismatch)
    {
        assert(mem.len <= (sizeof(mem.stack_lo) * 8 * 2) / 2 && "StepStack too small!");

        // Add state to memory
        mem.stack_hi = (mem.stack_hi << 2) | (mem.stack_lo >> (64 - 2));
        mem.stack_lo = (mem.stack_lo << 2) | static_cast<u64>(s.value);
    ...
    __device__ __forceinline__ Step pop()
    {
        ...
        // Remove state from memory
        mem.mismatches = mem.mismatches >> 1;
        mem.stack_lo = (mem.stack_lo >> 2) | (mem.stack_hi << (64 - 2));
        mem.stack_hi = (mem.stack_hi >> 2);
        mem.len -= 1;
    ...
    __device__ __forceinline__ Step current()
    {
        return Step{(Step::Inner)(mem.stack_lo & 0b11)};
    }
    ...
    /// Get the current CIGARX
    __device__ __forceinline__ Cigarx128 cigarx()
    {
        Cigarx128 result{};
        for (int i = mem.len - 1; i >= 0; --i)
        {
            ...
            Step step{(Step::Inner)(lo & 0b11)};
            bool mismatch = (mem.mismatches >> i & 0b1);
            result.push(step.to_cigarx_op_u128(mismatch));
        }
        return result;
    }
