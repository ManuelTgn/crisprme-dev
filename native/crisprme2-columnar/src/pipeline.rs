...
//! (implemented by [`ConnectorTx`](crate::pool::ConnectorTx) and
//! [`ConnectorRx`](crate::pool::ConnectorRx)).

use std::{sync::Arc, sync::atomic::{AtomicU64, Ordering}, thread::JoinHandle, time::Instant};

// =============================================================================
// Emit / Recv / Acquire traits
// =============================================================================

/// Stage error: the downstream channel is closed.
#[derive(Debug)]
...
where
    ...
    E2: Emit<B>,
{
    fn emit(&mut self, (a, b): (A, B)) -> Result<(), StageError> {
        self.0.emit(a)?;
        self.1.emit(b)?;
        Ok(())
    }
...
/// Object that can acquire fresh a T
pub trait Acquire<T> {
...
impl<C: Send + Sync + 'static> Pipeline<C> {
    ...
    where
        F: Fn(I, &mut E) -> Result<(), StageError> + Send + Sync + 'static,
...
mod test {
    use super::*;
    use crate::pool::{connector_mut, connector_ref, Pool};
